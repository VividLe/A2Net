...
class TALDataset(Dataset):
    def __init__(self, cfg, split):
        self.root = os.path.join(cfg.ROOT_DIR, cfg.DATASET.FEAT_DIR)
        self.split = split
    ...
    def __getitem__(self, idx):
        ...
        # feat_tem = cv2.resize(feat_tem, self.target_size, interpolation=cv2.INTER_NEAREST)
        # feat_tem = cv2.resize(feat_tem, self.target_size, interpolation=cv2.INTER_LINEAR)
        feat_spa = data['feat_spa']
        # feat_spa = cv2.resize(feat_spa, self.target_size, interpolation=cv2.INTER_NEAREST)
        ...
        if self.split == self.train_split:
            ...
            cls_label_af = data['cls_label']
            reg_label_af = data['reg_label']

            return feat_spa, feat_tem, action_padding, num_segment, cls_label_af, reg_label_af
        else:
...
if __name__ == '__main__':
    ...
    for feat_spa, feat_tem, action, num, cls_label_af, reg_label_af in train_loader:
