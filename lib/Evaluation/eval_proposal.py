...
def plot_metric(save_fig_path, average_nr_proposals, average_recall, recall, tiou_thresholds=np.linspace(0.5, 0.95, 10)):
...
#     file_dir = './output/result/'
#     file_set = os.listdir(file_dir)
#     file_set.sort()
#     for file_name in file_set:
#         result_file = os.path.join(file_dir, file_name)
...
#             f.write("%s\n" % file_name[:-5])
#             f.write("%.4f  %.4f  %.4f  %.4f\n" % (np.mean(uniform_recall_valid[:,0]),
#                                                       np.mean(uniform_recall_valid[:,4]),
#                                                       np.mean(uniform_recall_valid[:,9]),
#                                                       np.mean(uniform_recall_valid[:,-1])))
...
def evaluation_proposal():
    result_file = '/data/home/v-yale/ActionLocalization/output/BSN_proposals.json'
    # gt_file = '/data/home/v-yale/ActionLocalization/output/gt_mini.json'
    gt_file = "./Evaluation/data/activity_net.v1-3.min.json"
    save_fig_path = '/data/home/v-yale/ActionLocalization/output/figure.jpg'
    ...
    plot_metric(save_fig_path, uniform_average_nr_proposals_valid, uniform_average_recall_valid, uniform_recall_valid)
    print("AR@1 is \t", np.mean(uniform_recall_valid[:, 0]))
    print("AR@5 is \t", np.mean(uniform_recall_valid[:, 4]))
    print("AR@10 is \t", np.mean(uniform_recall_valid[:, 9]))
