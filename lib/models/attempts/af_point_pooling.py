...
class PointPooling(nn.Module):
    ...
    def forward(self, x):
        tem_length = x.size(2)
        data = list()
        for i in range(1, tem_length+1):
            x_part = x[:, :, :i]
            res_pool, _ = torch.max(x_part, dim=2)
            res_pool = torch.unsqueeze(res_pool, dim=-1)
            data.append(res_pool)
        feature = torch.cat(data, dim=-1)
...
class LocNet(nn.Module):
    ...
    def __init__(self, cfg, scale=False):
        ...
        self.scale = scale
        self.point_pooling = PointPooling()
    ...
    def forward(self, x):
        ...
        # use point pooling
        feat0_tmp = self.point_pooling(feat0)
        feat0 = feat0 + feat0_tmp
        feat1_tmp = self.point_pooling(feat1)
        feat1 = feat1 + feat1_tmp
        feat2_tmp = self.point_pooling(feat2)
        feat2 = feat2 + feat2_tmp
        feat3_tmp = self.point_pooling(feat3)
        feat3 = feat3 + feat3_tmp
        feat4_tmp = self.point_pooling(feat4)
        feat4 = feat4 + feat4_tmp
        feat5_tmp = self.point_pooling(feat5)
        feat5 = feat5 + feat5_tmp
        feat6_tmp = self.point_pooling(feat6)
        feat6 = feat6 + feat6_tmp
