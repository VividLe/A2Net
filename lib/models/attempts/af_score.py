...
class PredHeadBranch(nn.Module):
    ...
    def forward(self, x):
        ...
        pred_ori = self.pred(feat4)  # [batch, 2, temporal_length]
        pred = pred_ori.permute(0, 2, 1).contiguous()

        return pred, pred_ori
...
class PredScoreBranch(nn.Module):
    ...
    '''
    def __init__(self, cfg, in_channel=258, out_channel=1):
...
class PredHead(nn.Module):
    ...
    def forward(self, x):
        ...
        cls, _ = self.cls_branch(x)
        reg, reg_ori = self.reg_branch(x)
        return cls, reg, reg_ori
...
class LocNet(nn.Module):
    ...
    def _layer_cal(self, feat_list, scale_list):
        ...
        for feat, scale in zip(feat_list, scale_list):
            cls_tmp, reg_tmp, reg_ori_tmp = self.pred(feat)
            if self.scale:
                reg_tmp = scale(reg_tmp)
                reg_ori_tmp = scale(reg_ori_tmp)
            # predict confidence
            feat_cat = torch.cat((feat, reg_ori_tmp), dim=1)
