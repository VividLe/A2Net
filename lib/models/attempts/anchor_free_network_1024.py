...
class FeatNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.conv7 = conv1d(cfg, stride=2)
        self.conv8 = conv1d(cfg, stride=2)
        self.conv9 = conv1d(cfg, stride=2)
    ...
    def forward(self, x):
        ...
        feat8 = self.relu(self.conv8(feat7))
        feat9 = self.relu(self.conv9(feat8))

        # it is important in this order
        return feat0, feat1, feat2, feat3, feat4, feat5, feat6, feat7, feat8, feat9
...
class LocNet(nn.Module):
    ...
    def forward(self, x):
        ...
        scale_list = [self.scale0, self.scale1, self.scale2, self.scale3, self.scale4, self.scale5, self.scale6, self.scale7, self.scale8, self.scale9]
...
if __name__ == '__main__':
    ...
    cfg_file = '/disk2/zt/code/actloc_fixed_length_github/experiments/anet/af_1024_toy.yaml'
    update_config(cfg_file)

    model = LocNet(cfg, scale=True).cuda()
    data = torch.randn((2, 2048, 1024)).cuda()
    res = model(data)
