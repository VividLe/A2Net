...
class FeatNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return feat0, feat1, feat2, feat3, feat4, feat5, feat6, feat7
...
class LocNet(nn.Module):
    ...
    def forward(self, x):
        ...
        scale_list = [self.scale0, self.scale1, self.scale2, self.scale3, self.scale4, self.scale5, self.scale6, self.scale7]
...
if __name__ == '__main__':
    ...
    cfg_file = '/disk2/zt/code/actloc_fixed_length_github/experiments/anet/af_256_toy.yaml'
    update_config(cfg_file)

    model = LocNet(cfg, scale=True).cuda()
    data = torch.randn((2, 2048, 256)).cuda()
    res = model(data)
